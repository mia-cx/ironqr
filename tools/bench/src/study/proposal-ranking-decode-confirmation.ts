...
interface Config extends Record<string, unknown> {
  readonly variants: readonly ProposalRankingVariant[];
...
interface VariantResult {
  readonly variantId: ProposalRankingVariant;
...
interface Comparison {
  readonly variantId: ProposalRankingVariant;
  readonly comparedTo: ProposalRankingVariant;
...
const parseConfig = ({
  ...
  const variants =
    variantFlag.length === 0
      ? VARIANTS
      : variantFlag.split(',').map((id) => id.trim() as ProposalRankingVariant);
  const known = new Set<ProposalRankingVariant>(VARIANTS);
  for (const variant of variants)
    if (!known.has(variant)) throw new Error(`unknown proposal ranking variant: ${variant}`);
  if (!variants.includes('baseline'))
    throw new Error('proposal-ranking-decode-confirmation requires baseline');
  return {
    variants,
...
export const proposalRankingDecodeConfirmationStudyPlugin: StudyPlugin<
  Summary,
  Config,
  AssetResult
> = {
...
const runVariant = async (
  ...
  const results = await scanFrame(image, {
    ...
    proposalRankingVariant: variant,
    traceSink: trace,
...
const summarize = ({ config, results, cache }: StudySummaryInput<Config, AssetResult>): Summary => {
  ...
  return {
    ...
    recommendation: [
      ...
      'Use decode attempt, cluster, and timing deltas to decide whether prioritization reduces downstream work.',
    ],
  };
};

const summarizeVariant = (
  variantId: ProposalRankingVariant,
...
const numericFlag = (
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 1) {
    throw new Error(`proposal-ranking-decode-confirmation --${name} must be a positive integer`);
  }
  return value;
};
