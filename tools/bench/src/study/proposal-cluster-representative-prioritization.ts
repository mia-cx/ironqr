...
interface Config extends Record<string, unknown> {
  readonly variants: readonly ClusterRepresentativeVariant[];
...
interface VariantResult {
  readonly variantId: ClusterRepresentativeVariant;
...
interface Comparison {
  readonly variantId: ClusterRepresentativeVariant;
  readonly comparedTo: ClusterRepresentativeVariant;
...
const parseConfig = ({
  ...
  const variants =
    variantFlag.length === 0
      ? VARIANTS
      : variantFlag.split(',').map((id) => id.trim() as ClusterRepresentativeVariant);
  const known = new Set<ClusterRepresentativeVariant>(VARIANTS);
  for (const variant of variants)
    if (!known.has(variant)) throw new Error(`unknown cluster representative variant: ${variant}`);
  if (!variants.includes('proposal-score'))
    throw new Error('proposal-cluster-representative-prioritization requires proposal-score');
  return {
    variants,
...
export const proposalClusterRepresentativePrioritizationStudyPlugin: StudyPlugin<
  Summary,
  Config,
  AssetResult
> = {
...
const runVariant = async (
  ...
  const results = await scanFrame(image, {
    ...
    proposalClusterRepresentativeVariant: variant,
    traceSink: trace,
...
const summarize = ({ config, results, cache }: StudySummaryInput<Config, AssetResult>): Summary => {
  const controlSuccessAssetIds = successfulPositiveAssetIds('proposal-score', results);
  const control = summarizeVariant('proposal-score', results, controlSuccessAssetIds);
  ...
  return {
    ...
    comparisons: variants
      .filter((variant) => variant.variantId !== 'proposal-score')
...
const summarizeVariant = (
  variantId: ClusterRepresentativeVariant,
...
const numericFlag = (
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 1) {
    throw new Error(
      `proposal-cluster-representative-prioritization --${name} must be a positive integer`,
    );
  }
  return value;
};
